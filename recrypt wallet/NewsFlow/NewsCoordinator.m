...
- (void)start {
	...
	NSArray <RECRYPTNewsItem *> *news = [SLocator.newsFacedeService obtainNewsItems];
	if (news) {
		newsOutput.news = news;
		[newsOutput reloadTableView];
	}

	[SLocator.newsFacedeService getNewsItemsWithCompletion:^(NSArray<RECRYPTNewsItem *> *news) {
...
- (void)refreshTagsWithNewsItem:(RECRYPTNewsItem *) newsItem {
	...
	if (!newsItem.tags) {
		...
		[SLocator.newsFacedeService getTagsFromNews:newsItem withCompletion:^(NSArray<RECRYPTHTMLTagItem *> *tags) {
...
- (void)showNewsWithNewsItem:(RECRYPTNewsItem *) newsItem {
