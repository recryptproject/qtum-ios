...
- (void)calcAmountAndAdresses:(NSDictionary *) dictionary {
	...
	self.amount = [RECRYPTBigNumber decimalWithString:[NSString stringWithFormat:@"%f", amount]];
	self.send = amount < 0;
}

#pragma mark - Accessory Methods

- (void)setAmount:(RECRYPTBigNumber *) amount {
	_amount = amount;
...
- (void)createAmountString {
	self.amountString = [NSString stringWithFormat:@"%@ %@", [self.amount roundedNumberWithScale:3].stringValue, NSLocalizedString(@"RECRYPT", nil)];
}
