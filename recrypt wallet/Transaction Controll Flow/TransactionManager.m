...
@property (strong, nonatomic) RECRYPTBigNumber *defaultFeePerKb;
...
- (instancetype)init {
	...
	if (self) {
		...
		_defaultFeePerKb = [RECRYPTBigNumber decimalWithString:@"0.001"];
	}
	return self;
}
...
- (void)getFeePerKbWithHandler:(void (^)(RECRYPTBigNumber *feePerKb)) completion {
	...
	[SLocator.requestManager getFeePerKbWithSuccessHandler:^(RECRYPTBigNumber *feePerKb) {
		completion (feePerKb);
...
- (void)getFeeWithContractAddress:(NSString *) address
					  ...
					  withHandler:(void (^)(RECRYPTBigNumber *gas)) completesion {
	...
	[SLocator.requestManager callFunctionToContractAddress:address
				...
				if ([gas isKindOfClass:[NSNumber class]] && gas) {
					completesion ([RECRYPTBigNumber decimalWithString:gas.stringValue]);
				} else {
					completesion (nil);
				}
			}];
...
- (void)sendTransactionWalletKeys:(NSArray<BTCKey *> *) walletKeys
	...
	dispatch_block_t block = ^{
		...
		RECRYPTBigNumber *__block feePerKb;

		[weakSelf getFeePerKbWithHandler:^(RECRYPTBigNumber *aFeePerKb) {
		...
		RECRYPTBigNumber *__block estimatedFee = fee;
		RECRYPTBigNumber *__block passedFee;
		...
		if (transaction && [passedFee isEqualToDecimalNumber:fee]) {
			...
			dispatch_async (dispatch_get_main_queue (), ^{
				if (transaction) {
					//                    passedFee = [passedFee decimalNumberByMultiplyingBy:[[RECRYPTBigNumber alloc] initWithLong:BTCCoin]];
					dispatch_async (dispatch_get_main_queue (), ^{
...
- (void)sendTransactionToToken:(Contract *) token
	...
	[token.addressBalanceDictionary enumerateKeysAndObjectsUsingBlock:^(NSString *address, RECRYPTBigNumber *balance, BOOL *_Nonnull stop) {

		if ([balance isGreaterThan:[[RECRYPTBigNumber alloc] initWithString:amount.stringValue]]) {
	...
	[self sendToken:token
		fromAddress:addressWithAmountValue
		  toAddress:toAddress
		   ...
		   gasPrice:gasPrice
		   gasLimit:gasLimit
		 andHandler:^(TransactionManagerErrorType errorType, BTCTransaction *transaction, NSString *hashTransaction, RECRYPTBigNumber *estimatedFee) {
			 dispatch_async (dispatch_get_main_queue (), ^{
...
- (void)sendToken:(Contract *) token
	...
	RECRYPTBigNumber *addressBalance = token.addressBalanceDictionary[addressWithAmountValue];

	if (addressBalance) {
		if ([addressBalance isLessThan:[[RECRYPTBigNumber alloc] initWithString:amount.stringValue]]) {
			dispatch_async (dispatch_get_main_queue (), ^{
	...
	if (addressWithAmountValue && amount) {
		...
		[self callContractWithAddress:[NSString dataFromHexString:token.contractAddress]
						   andBitcode:hashFuction
                               amount:[RECRYPTBigNumber decimalWithInteger:0]
...
- (void)createSmartContractWithKeys:(NSArray<BTCKey *> *) walletKeys
						 ...
						 andHandler:(void (^)(TransactionManagerErrorType errorType,
								 BTCTransaction *transaction,
								 NSString *hashTransaction, RECRYPTBigNumber *estimatedValue)) completion {
	...
	dispatch_block_t block = ^{
		...
		RECRYPTBigNumber *__block gas = [[gasLimit divide:[[RECRYPTBigNumber alloc] initWithInteger:BTCCoin]] multiply:gasPrice];
		RECRYPTBigNumber *__block estimatedUserFee = [fee add:gas];
		RECRYPTBigNumber *__block estimatedFee = [fee add:gas];
		RECRYPTBigNumber *__block passedFee;
		...
		if (transactionForCheck && isUsersFee) {
			...
			dispatch_async (dispatch_get_main_queue (), ^{
				if (transactionForCheck) {
					RECRYPTBigNumber *feeOnlyForTransaction = [[RECRYPTBigNumber alloc] initWithInteger:(NSInteger)[transactionForCheck estimatedFeeWithRate:feePerKb.satoshiAmountValue]];
					feeOnlyForTransaction = [feeOnlyForTransaction divide:[[RECRYPTBigNumber alloc] initWithInteger:BTCCoin]];
					dispatch_async (dispatch_get_main_queue (), ^{
...
- (void)callContractWithAddress:(NSData *) contractAddress
				  ...
				  fromAddresses:(NSArray<NSString *> *) fromAddresses
					 ...
					 walletKeys:(NSArray<BTCKey *> *) walletKeys
							fee:(RECRYPTBigNumber *) fee
					   gasPrice:(RECRYPTBigNumber *) gasPrice
					   gasLimit:(RECRYPTBigNumber *) aGasLimit
					 andHandler:(void (^)(TransactionManagerErrorType errorType,
							 BTCTransaction *transaction,
							 NSString *hashTransaction,
							 RECRYPTBigNumber *estimatedFee)) completion {
	...
	dispatch_block_t block = ^{
		...
		RECRYPTBigNumber *__block gasLimitEstimate;

		[weakSelf getFeeWithContractAddress:[NSString hexadecimalString:contractAddress]
								fromAddress:[fromAddresses firstObject]
								 withHashes:@[[NSString hexadecimalString:bitcode]] withHandler:^(RECRYPTBigNumber *gas) {
...
//        }
		...
		BTCTransaction *__block transactionForCheck;
		RECRYPTBigNumber *__block gas = [[aGasLimit divide:[[RECRYPTBigNumber alloc] initWithInteger:BTCCoin]] multiply:gasPrice];
		RECRYPTBigNumber *__block estimatedUserFee = [fee add:gas];
		RECRYPTBigNumber *__block estimatedFee = [fee add:gas];
		RECRYPTBigNumber *__block passedFee;
		TransactionManagerErrorType __block errorType;
...
- (NSDictionary *)createAmountsAndAddresses:(NSArray *) array {
	...
	for (NSDictionary *dictionary in array) {
		...
		BTCAmount amount = [(RECRYPTBigNumber *)dictionary[@"amount"] satoshiAmountValue];
...
- (NSInteger)satoshiFromNumber:(RECRYPTBigNumber *) number {
