...
-(void)updatePublishedContracts {
    ...
    if (self.publisedContractsOutput) {
        ...
        dispatch_async(dispatch_queue_create("com.example.recrypt.contract.update", DISPATCH_QUEUE_CONCURRENT), ^{
...
- (void)finishStepFinishDidPressed:(RECRYPTBigNumber *) fee gasPrice:(RECRYPTBigNumber *) gasPrice gasLimit:(RECRYPTBigNumber *) gasLimit {
    ...
    [SLocator.callContractFacadeService createSmartContractWithTemplate:self.templateModel.path
                                                               ...
                                                               gasLimit:gasLimit
                                                             andHandler:^(TransactionManagerErrorType errorType, BTCTransaction *transaction, NSString *hashTransaction, RECRYPTBigNumber *estimatedValue) {
                                                                 [SLocator.popupService dismissLoader];
                                                                 if (errorType == TransactionManagerErrorTypeNone) {
                                                                     ...
                                                                     if (errorType == TransactionManagerErrorTypeNotEnoughFee) {
                                                                         errorString = [NSString stringWithFormat:@"Insufficient fee. Please use minimum of %@ RECRYPT", estimatedValue];
                                                                     }
                                                                     if (errorType == TransactionManagerErrorTypeNotEnoughGasLimit) {
                                                                         errorString = [NSString stringWithFormat:@"Insufficient gas limit. Please use minimum of %@ RECRYPT", estimatedValue];
                                                                     }
                                                                     switch (errorType) {
                                                                         ...
                                                                         case TransactionManagerErrorTypeInvalidAddress:
                                                                             [weakSelf.createFinishViewController showErrorPopUp:NSLocalizedString(@"Invalid RECRYPT Address", nil)];
                                                                             break;
...
- (void)showStatusOfPayment:(TransactionManagerErrorType) errorType {
	...
	switch (errorType) {
		...
		case TransactionManagerErrorTypeNotEnoughMoney:
			...
			break;
		case TransactionManagerErrorTypeInvalidAddress:
			[self.functionDetailController showErrorPopUp:NSLocalizedString(@"Invalid RECRYPT Address", nil)];
			break;
		case TransactionManagerErrorTypeNotEnoughMoneyOnAddress:
...
- (void)showStatusOfPayment:(TransactionManagerErrorType) errorType withEstimateFee:(RECRYPTBigNumber *) estimatedFee {
