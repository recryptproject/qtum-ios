...
- (void)showNotEnoughFeeAlertWithEstimatedFee:(NSDecimalNumber *) estimatedFee {
	...
	NSString *errorString = [NSString stringWithFormat:@"Insufficient fee. Please use minimum of %@ RECRYPT", estimatedFee];
...
- (void)didChangeFeeSlider:(UISlider *) slider {
	...
	RECRYPTBigNumber *bigNumber = [RECRYPTBigNumber decimalWithString:sliderValue.stringValue];
	self.FEE = bigNumber;
	self.feeView.feeTextField.text = self.FEE.stringValue;
}
...
- (void)normalizeFee {
	...
	RECRYPTBigNumber *feeValue = [RECRYPTBigNumber decimalWithString:self.feeView.feeTextField.text];
...
- (IBAction)didPressedCallAction:(id) sender {
    ...
    RECRYPTBigNumber* amount;
    if (self.amountTextField.text.length > 0) {
        amount = [RECRYPTBigNumber decimalWithString:self.amountTextField.text];
    } else {
