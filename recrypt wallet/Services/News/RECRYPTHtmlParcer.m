...
@interface RECRYPTHtmlParcer ()

@property (nonatomic, copy) RECRYPTTagsItems completion;
@property (nonatomic, strong) NSOperationQueue *workingQueue;
@property (nonatomic, strong) TFHpple *parser;
@end

@implementation RECRYPTHtmlParcer
...
- (NSArray<RECRYPTHTMLTagItem *> *)createThreeOfTagsWith:(NSArray<TFHppleElement *> *) elements andOperation:(NSBlockOperation *) operation {

	if (operation.isCancelled) {
		return nil;
	}

	NSMutableArray<RECRYPTHTMLTagItem *> *tagsItems = @[].mutableCopy;
	...
	for (TFHppleElement *happleElement in elements) {
		...
		RECRYPTHTMLTagItem *tag = [[RECRYPTHTMLTagItem alloc] init];
