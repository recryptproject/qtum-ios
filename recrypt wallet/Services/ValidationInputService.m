...
- (BOOL)isValidAmountString:(NSString*)string {
    ...
    NSArray<NSTextCheckingResult *> *matches = [regex matchesInString:string options:0 range:NSMakeRange (0, string.length)];
    BOOL match = matches.count > 0;
    RECRYPTBigNumber* result = [[RECRYPTBigNumber decimalWithString:string] multiply:[RECRYPTBigNumber decimalWithInteger:BTCCoin]];
    BOOL comparing = ![result isGreaterThan:[RECRYPTBigNumber maxBigNumberWithPowerOfTwo:64 isUnsigned:NO]];
...
- (BOOL)isValidUIntFromString:(NSString*) string withSize:(NSInteger) size withRegex:(NSRegularExpression*) regex {
    ...
    NSArray<NSTextCheckingResult *> *matches = [regex matchesInString:string options:0 range:NSMakeRange (0, string.length)];
    BOOL match = matches.count > 0;
    BOOL comparing = ![[RECRYPTBigNumber decimalWithString:string] isGreaterThan:[RECRYPTBigNumber maxBigNumberWithPowerOfTwo:size isUnsigned:YES]];
    return match && comparing;
}
...
- (BOOL)isValidIntFromString:(NSString*) string withSize:(NSInteger) size withRegex:(NSRegularExpression*) regex {
    ...
    NSArray<NSTextCheckingResult *> *matches = [regex matchesInString:string options:0 range:NSMakeRange (0, string.length)];
    BOOL match = matches.count > 0;
    BOOL comparing = ![[RECRYPTBigNumber decimalWithString:string] isGreaterThan:[RECRYPTBigNumber maxBigNumberWithPowerOfTwo:size isUnsigned:NO]];
    return match && comparing;
}
