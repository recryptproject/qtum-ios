...
- (NSArray <WalletBalancesObject *> *)prepareAddresesDataWithResponse:(NSArray *) response {
	...
	for (int i = 0; i < allAddressesArray.count; i++) {
		...
		object.longBalanceStringBalance = [[RECRYPTBigNumber decimalWithInteger:0] stringValue];
		object.shortBalanceStringBalance = [[RECRYPTBigNumber decimalWithInteger:0] stringValue];
	...
	for (NSDictionary *item in response) {
		...
		RECRYPTBigNumber *bigAmount = [RECRYPTBigNumber decimalWithString:amountNumber.stringValue];
		...
		if (address && amountDouble > 0) {
			...
			RECRYPTBigNumber *addressAmountDecimalConteiner = [RECRYPTBigNumber decimalWithString:object.longBalanceStringBalance];
			RECRYPTBigNumber *newBalance = [addressAmountDecimalConteiner add:bigAmount];
...
- (void)makeTransferFromAddress:(NSString *) from toAddress:(NSString *) to withAmount:(NSString *) amount {
	...
	SendInfoItem *item = [[SendInfoItem alloc] initWithRecryptAddressKey:self.addressKeyHashTable[to] tokenAddress:nil amountString:amount fromRecryptAddressKey:self.addressKeyHashTable[from]];
