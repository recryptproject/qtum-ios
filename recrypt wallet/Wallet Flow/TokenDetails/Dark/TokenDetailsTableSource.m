...
static CGFloat const RECRYPTAddressTokenHeight = 47;
...
static NSString *const RECRYPTAddressTokenIdentifier = @"RECRYPTAddressTokenTableViewCell";
...
@interface TokenDetailsTableSource () <RECRYPTAddressTokenTableViewCellDelegate, RECRYPTAddressTokenViewDelegate>
...
- (CGFloat)tableView:(UITableView *) tableView heightForRowAtIndexPath:(NSIndexPath *) indexPath {
	if (indexPath.section == 0) {
		...
		height += RECRYPTAddressTokenHeight;
...
- (UITableViewCell *)getCellForFirstSection:(UITableView *) tableView forRow:(NSInteger) row {
	...
	view = [cell addViewOrReturnContainViewForUpdate:[self createOrUpdateRECRYPTAddressTokenView:nil] withHeight:RECRYPTAddressTokenHeight];
	if (!view)
		[self createOrUpdateRECRYPTAddressTokenView:(RECRYPTAddressTokenView *)view];
...
- (RECRYPTAddressTokenView *)createOrUpdateRECRYPTAddressTokenView:(RECRYPTAddressTokenView *) view {

	RECRYPTAddressTokenView *updatedView = view;
	if (!updatedView) {
		updatedView = (RECRYPTAddressTokenView *)[self getViewFromXib:[RECRYPTAddressTokenView class]];
	}
...
#pragma mark - RECRYPTAddressTokenTableViewCellDelegate
