...
- (void)reloadHeader:(id <Spendable>) wallet {
	...
	self.uncorfirmedLabel.text = [NSString stringWithFormat:@"%@ %@", [wallet.unconfirmedBalance roundedNumberWithScale:3], NSLocalizedString(@"RECRYPT", nil)];
	self.availabelLabel.text = [NSString stringWithFormat:@"%@ %@", [wallet.balance roundedNumberWithScale:3], NSLocalizedString(@"RECRYPT", nil)];
}
