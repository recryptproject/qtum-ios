...
NSString const *kWallets = @"recrypt_wallet_wallets_keys";
NSString const *kSingleWallet = @"recrypt_wallet_wallet_keys";
...
- (void)updateBalanceOfSpendableObject:(Wallet <Spendable> *) object withHandler:(void (^)(BOOL success)) complete {
	...
	[self.requestAdapter getBalanceForAddreses:[object allKeysAdreeses] withSuccessHandler:^(RECRYPTBigNumber *balance) {
...
- (void)updateSpendablesBalansesWithObject:(NSDictionary *) balances {
	...
	if ([balance isKindOfClass:[NSNumber class]]) {
		self.wallet.balance = [RECRYPTBigNumber decimalWithString:balance.stringValue];
	}

	if ([unconfirmedBalance isKindOfClass:[NSNumber class]]) {
		self.wallet.unconfirmedBalance = [RECRYPTBigNumber decimalWithString:unconfirmedBalance.stringValue];
	}
