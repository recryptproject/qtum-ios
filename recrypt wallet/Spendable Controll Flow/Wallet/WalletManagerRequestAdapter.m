...
- (void)getBalanceForAddreses:(NSArray *) keyAddreses
		   withSuccessHandler:(void (^)(RECRYPTBigNumber *balance)) success
...
- (RECRYPTBigNumber *)calculateBalance:(NSArray *) responseObject {

	RECRYPTBigNumber *balance = [RECRYPTBigNumber decimalWithInteger:0];

	for (NSDictionary *dictionary in responseObject) {

		NSNumber *amount = dictionary[@"amount"];
		if ([amount isKindOfClass:[NSNumber class]]) {
			balance = [balance add:[RECRYPTBigNumber decimalWithString:amount.stringValue]];
		}
	}
