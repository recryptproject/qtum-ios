...
- (instancetype)initWithName:(NSString *) name pin:(NSString *) pin {
	...
	if (self) {
		_name = name;
		_balance = [RECRYPTBigNumber decimalWithInteger:0];
		_unconfirmedBalance = [RECRYPTBigNumber decimalWithInteger:0];
...
- (NSString *)symbol {
	...
	return NSLocalizedString(@"RECRYPT", @"RECRYPT sybmol");
}
