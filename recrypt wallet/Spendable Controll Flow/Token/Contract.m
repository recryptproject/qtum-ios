...
- (instancetype)init {
	...
	if (self) {
		...
		_unconfirmedBalance = [[RECRYPTBigNumber alloc] initWithString:@"0"];
	}
	return self;
}
...
- (void)setAddressBalanceDictionary:(NSDictionary<NSString *, RECRYPTBigNumber *> *) addressBalanceDictionary {
...
- (RECRYPTBigNumber *)balance {

	if (!_balance) {
		NSArray *values = self.addressBalanceDictionary.allValues;
		RECRYPTBigNumber *balanceDecimal = [[RECRYPTBigNumber alloc] initWithString:@"0"];

		for (RECRYPTBigNumber *balanceValue in values) {
...
- (nullable instancetype)initWithCoder:(NSCoder *) aDecoder {
	...
	RECRYPTBigNumber *decimals = [aDecoder decodeObjectForKey:kContractDecimals];
	RECRYPTBigNumber *totalSupply = [aDecoder decodeObjectForKey:kContractTotalSupply];
	...
	if (self) {
		...
		_adresses = adresses;
		_symbol = symbol;
		_decimals = [decimals isKindOfClass:[NSNumber class]] ? [[RECRYPTBigNumber alloc] initWithString:[(NSNumber *)decimals stringValue]] : decimals;
		_totalSupply = [totalSupply isKindOfClass:[NSNumber class]] ? [[RECRYPTBigNumber alloc] initWithString:[(NSNumber *)decimals stringValue]] : totalSupply;
		_isActive = isActive;
        _addressBalanceDictionary = addressBalanceDictionary;
	}
