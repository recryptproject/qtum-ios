...
NSString *const kTokenKeys = @"recrypt_token_tokens_keys";
...
-(NSMutableDictionary <NSString *, RECRYPTBigNumber *> *)emptyTokenAddressBalancesDict {
    
    NSArray* addresses = SLocator.walletManager.wallet.addressesInRightOrder;
    NSMutableDictionary <NSString *, RECRYPTBigNumber *> * emptyTokensBalances = @{}.mutableCopy;
    
    for (int i = 0; i < addresses.count; i++) {
        RECRYPTBigNumber* zeroValue = [RECRYPTBigNumber decimalWithInteger:0];
		...
		for (NSDictionary *dict in addressBalance[@"balances"]) {
			...
			if (balance) {
				if ([balance isKindOfClass:[NSNumber class]]) {
					newAddressBalance[addressKey] = [[RECRYPTBigNumber alloc] initWithString:[(NSNumber *)balance stringValue]];
				} else if ([balance isKindOfClass:[NSString class]]) {
					newAddressBalance[addressKey] = [[RECRYPTBigNumber alloc] initWithString:balance];
				}
			}
		}
