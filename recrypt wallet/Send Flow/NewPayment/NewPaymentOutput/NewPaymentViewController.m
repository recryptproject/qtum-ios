...
@property (strong, nonatomic) RECRYPTBigNumber *FEE;
@property (strong, nonatomic) RECRYPTBigNumber *gasPrice;
@property (strong, nonatomic) RECRYPTBigNumber *gasLimit;
@property (strong, nonatomic) RECRYPTBigNumber *minFee;
@property (strong, nonatomic) RECRYPTBigNumber *maxFee;
...
- (void)viewDidLoad {
	...
	self.tokenTextField.text = NSLocalizedString(@"RECRYPT (Default)", @"");
...
- (void)configFee {
	...
	self.FEE = [RECRYPTBigNumber decimalWithString:@"0.1"];
...
- (void)configGasPrice {
	...
	self.gasPrice = [RECRYPTBigNumber decimalWithString:@"40"];
...
- (void)configGasLimit {
	...
	self.gasLimit = [RECRYPTBigNumber decimalWithString:@"400000000"];
...
- (void)normalizeFee {
	...
	RECRYPTBigNumber *feeValue = [RECRYPTBigNumber decimalWithString:self.feeTextField.text];
...
- (void)updateWithEtity:(NewPaymentOutputEntity *) entity {
    ...
    if (entity.amount) {
        ...
        if (entity.amount && ![entity.amount isEqualToString:@""]) {
            RECRYPTBigNumber *amount = [RECRYPTBigNumber decimalWithString:entity.amount];
    ...
    if (!entity.isTokenChoosen) {
        ...
        self.tokenTextField.text = NSLocalizedString(@"RECRYPT (Default)", nil);
...
- (void)updateWithTokenChoosen:(BOOL) isChoosen andTokenName:(NSString*) tokenName {
    ...
    if (self.tokenTextField) {
        self.tokenTextField.text = isChoosen ? tokenName: NSLocalizedString(@"RECRYPT (Default)", @"");
        self.needUpdateTokenTexfFields = NO;
    } else {
...
- (void)updateQuickInfoOfWalletWithBalance:(RECRYPTBigNumber*) balance andUnconfirmedBalance:(RECRYPTBigNumber*) unconfirmedBalance {
    ...
    self.balanceSymbolLabel.text = NSLocalizedString(@"RECRYPT", @"");
...
- (void)setMinGasPrice:(RECRYPTBigNumber *) min andMax:(RECRYPTBigNumber *) max step:(long) step {
	...
	self.gasPrice = [RECRYPTBigNumber decimalWithString:min.stringValue];
...
- (void)setMinGasLimit:(RECRYPTBigNumber *) min andMax:(RECRYPTBigNumber *) max standart:(RECRYPTBigNumber *) standart step:(long) step; {
	...
	self.gasLimit = [RECRYPTBigNumber decimalWithString:standart.stringValue];
...
- (BOOL)textField:(UITextField *) textField shouldChangeCharactersInRange:(NSRange) range replacementString:(NSString *) string {
    ...
    if (textField == self.amountTextField) {
        ...
        if (isValid) {
            ...
            RECRYPTBigNumber *feeValue = [RECRYPTBigNumber decimalWithString:textField.text];
...
- (IBAction)makePaymentButtonWasPressed:(id) sender {
    ...
    RECRYPTBigNumber *amount = [RECRYPTBigNumber decimalWithString:self.amountTextField.text];
...
- (IBAction)didChangeFeeSlider:(UISlider *) slider {
    ...
    self.FEE = [RECRYPTBigNumber decimalWithString:sliderValue.stringValue];
...
- (IBAction)didChangeGasPriceSlider:(UISlider *) slider {
    ...
    self.gasPrice = [RECRYPTBigNumber decimalWithString:sliderValue.stringValue];
...
- (IBAction)didChangeGasLimitSlider:(UISlider *) slider {
    ...
    NSDecimalNumber *sliderValue = [[NSDecimalNumber alloc] initWithUnsignedLong:value];
    self.gasLimit = [RECRYPTBigNumber decimalWithString:sliderValue.stringValue];
