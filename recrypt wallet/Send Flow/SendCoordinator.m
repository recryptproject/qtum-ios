...
- (void)start {
	...
	[SLocator.transactionManager getFeePerKbWithHandler:^(RECRYPTBigNumber *feePerKb) {

		RECRYPTBigNumber *minFee = [feePerKb roundedNumberWithScale:5];
		RECRYPTBigNumber *maxFee = SLocator.paymentValuesManager.maxFee;
...
- (void)setForSendSendInfoItem:(SendInfoItem *) item {
	...
	self.fromAddressKey = item.fromRecryptAddressKey;
    
    if (item.fromRecryptAddress) {
        self.fromAddressString = item.fromRecryptAddress;
        ...
        for (ContracBalancesObject* info in tokenBalanceInfo) {
            if ([info.addressString isEqualToString:item.fromRecryptAddress]) {
...
- (void)setForToken:(Contract *) aToken withAddress:(NSString *) address andAmount:(NSString *) amount {
	...
	SendInfoItem *item = [[SendInfoItem alloc] initWithRecryptAddress:address tokenAddress:aToken.contractAddress amountString:amount];
...
-(NewPaymentOutputEntity*)entityWithSendInfo:(SendInfoItem*) item {
    ...
    entity.receiverAddress = item.recryptAddressKey ? [SLocator.walletManager stringAddressFromBTCKey:item.recryptAddressKey] : item.recryptAddress;
...
- (void)payWithWalletWithAddress:(NSString *) address andAmount:(RECRYPTBigNumber *) amount andFee:(RECRYPTBigNumber *) fee {
...
- (void)payWithTokenWithAddress:(NSString *) address andAmount:(RECRYPTBigNumber *) amount fee:(RECRYPTBigNumber *) fee gasPrice:(RECRYPTBigNumber *) gasPrice gasLimit:(RECRYPTBigNumber *) gasLimit {

	RECRYPTBigNumber *amounDivByDecimals = [amount numberWithPowerOf10:self.token.decimals];
...
- (void)showStatusOfPayment:(TransactionManagerErrorType) errorType {
	...
	switch (errorType) {
		...
		case TransactionManagerErrorTypeInvalidAddress:
			[self showErrorPopUp:NSLocalizedString(@"Invalid RECRYPT Address", nil)];
			break;
...
- (void)showNotEnoughFeeAlertWithEstimatedFee:(RECRYPTBigNumber *) estimatedFee {
	...
	NSString *errorString = [NSString stringWithFormat:@"Insufficient fee. Please use minimum of %@ RECRYPT", estimatedFee.stringValue];
	[self showErrorPopUp:NSLocalizedString(errorString, nil)];
}

- (void)showStatusOfPayment:(TransactionManagerErrorType) errorType withEstimateGasLimit:(RECRYPTBigNumber *) gasLimit {

	NSString *errorString = [NSString stringWithFormat:@"Insufficient gas limit. Please use minimum of %@ RECRYPT", gasLimit.stringValue];
	[self showErrorPopUp:NSLocalizedString(errorString, nil)];
}

- (BOOL)isValidAmount:(RECRYPTBigNumber *) amount {
...
- (void)didPresseSendActionWithAddress:(NSString *) address andAmount:(RECRYPTBigNumber *) amount fee:(RECRYPTBigNumber *) fee gasPrice:(RECRYPTBigNumber *) gasPrice gasLimit:(RECRYPTBigNumber *) gasLimit {
...
- (void)payActionWithAddress:(NSString *) address andAmount:(RECRYPTBigNumber *) amount fee:(RECRYPTBigNumber *) fee gasPrice:(RECRYPTBigNumber *) gasPrice gasLimit:(RECRYPTBigNumber *) gasLimit {
